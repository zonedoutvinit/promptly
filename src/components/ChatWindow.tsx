        ...
        {!hasMessages ? (
          ...
          messages.map((msg, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`flex flex-col p-4 rounded-xl w-fit border animate-messageSlide ${
                  msg.role === "user"
              ...
              >
                ...
                {msg.role === "assistant" ? (
                  <div className="prose prose-invert prose-sm max-w-none text-zinc-200 leading-relaxed text-justify">
                    <ReactMarkdown
                      components={{
                        // This injects the custom cursor smoothly inline directly following content tokens
                        p: ({ children }) => (
                          <p className="mb-2 last:mb-0">
                            {children}
                            {showCursor && (
                              <span className="inline-block ml-0.5 text-indigo-400 animate-cursorPulse">
                                ▋
                              </span>
                            )}
                          </p>
                    ...
                    >
                      {/* Standard fallback if the stream is starting up but content is blank */}
                      {msg.content || (!msg.content && showCursor ? "" : "")}
                    </ReactMarkdown>
                  </div>
                ) : (
